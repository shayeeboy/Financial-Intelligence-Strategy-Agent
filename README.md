...
| **R6 ✅ SHIPPED** | **Trend & forecast-free deltas** — live 2026-08-21 | Every indicator shows a directional period-over-period delta (▲/▼/▬ + %) and a unicode sparkline — in the web brief's new **Trend** column (sharing each row's citation) and per-indicator in the Studio snapshot (`delta` + `sparkline`). Descriptive only, no forecasting; BoC history bumped 2→13 for real rate sparklines. `AC-D1/D2/D3` tested | directional delta + multi-year sparkline on every headline figure — **verified live** |
